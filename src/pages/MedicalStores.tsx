...
const MedicalStores = () => {
  ...
  const filtered = useMemo(() => {
    let stores = medicalStores
      .map((store) => {
        if (userLocation) {
          const dist = getDistance(userLocation.lat, userLocation.lng, store.latitude, store.longitude);
          return { ...store, distance: `${dist.toFixed(1)} mi`, _distNum: dist };
        }
        return { ...store, _distNum: parseFloat(store.distance) || 999 };
      })
      .filter((store) => {
        const matchesSearch =
    ...
    if (userLocation) {
      stores = stores.sort((a, b) => a._distNum - b._distNum);
    }
